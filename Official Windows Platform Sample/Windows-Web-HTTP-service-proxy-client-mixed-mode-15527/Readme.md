...
<h1><a id="gallery_samples.httpcalculatorwithkerberosoversslclient_gallery"></a>Windows Web Services HTTP service proxy client mixed-mode authentication sample</h1>
</div>
<p>This sample shows how to use an HTTP service proxy client with the <a href="http://msdn.microsoft.com/en-us/library/windows/desktop/dd430435">
Windows Web Services API</a> to talk to a calculator service with Kerberos authentication and SSL mixed-mode transport security. In this setup, the transport connection is protected (signed, encrypted) by SSL which also provides server authentication. Client
 authentication is provided by a kerberos APREQ ticket in a WS-Security header in the message.</p>
