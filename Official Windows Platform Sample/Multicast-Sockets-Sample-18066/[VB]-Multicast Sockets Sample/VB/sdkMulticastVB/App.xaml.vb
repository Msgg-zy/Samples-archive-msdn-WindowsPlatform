...
#End If
    ...
    Private Sub Application_Launching(ByVal sender As Object, ByVal e As LaunchingEventArgs)
        ' Add handlers for all events from the GamePlay module
    End Sub
    ...
    Private Sub Application_Deactivated(ByVal sender As Object, ByVal e As DeactivatedEventArgs)
        If GamePlay IsNot Nothing Then
            GamePlay.Leave(True)
            _gamePlay = Nothing
        End If
    End Sub
...
#End Region
    ''' <summary>
    ''' List of players currently in the multicast group.
    ''' </summary>
    ''' <remarks>
    ''' This list changes when an event is received informing us that a player 
    ''' has joined or that a player has left.
    ''' </remarks>
    Public Shared Players As New ObservableCollection(Of PlayerInfo)()

    Private Shared _gamePlay As GamePlay = Nothing
    Public Shared ReadOnly Property GamePlay() As GamePlay
        Get
            ' Delay creation of GamePlay until necessary
            If _gamePlay Is Nothing Then
