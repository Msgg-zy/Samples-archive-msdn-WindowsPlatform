...
Windows Web Services API</a> to implement a TCP client that sends one-way messages in a stream. This sample performs streaming only on the application layer and data on the transport layer are buffered before they are returned to the client. The streaming mode
 is supported only on HTTP channels. </p>
