...
Windows Web Services API</a> to use the service proxy to talk to an HTTP based PurchaseOrder service. We also demonstrate how to configure the wsutil tool to generate the stub files with WS_STRING type instead of WCHAR* for strings in the XML schema and interface
 definitions in XSD/WSDL files using /string:WS_STRING command line option. </p>
