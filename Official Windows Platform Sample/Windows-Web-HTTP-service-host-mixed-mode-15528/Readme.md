...
<h1><a id="gallery_samples.httpcalculatorwithkerberosoversslservice_gallery"></a>Windows Web Services HTTP service host mixed-mode authentication sample</h1>
</div>
...
Windows Web Services API</a> to host a calculator service over HTTP with Kerberos authentication over SSL mixed-mode transport security. In this setup, the transport connection is protected (signed, encrypted) by SSL which also provides server authentication.
 Client authentication is provided by a kerberos APREQ ticket in a WS-Security header in the message.</p>
