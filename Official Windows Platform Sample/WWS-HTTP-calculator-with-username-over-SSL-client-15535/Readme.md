...
Windows Web Services API</a> to implement an HTTP client that uses the service proxy to talk to a calculator service with username over SSL mixed-mode security. In this setup, the transport connection is protected (signed, encrypted) by SSL which also provides
