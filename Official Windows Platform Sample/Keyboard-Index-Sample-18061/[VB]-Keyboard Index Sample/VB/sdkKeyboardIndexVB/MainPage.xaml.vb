'
'   Copyright (c) 2011 Microsoft Corporation.  All rights reserved.
'   Use of this sample source code is subject to the terms of the Microsoft license 
'   agreement under which you licensed this sample source code and is provided AS-IS.
'   If you did not accept the terms of the license agreement, you are not authorized 
'   to use this sample source code.  For the terms of the license, please see the 
'   license agreement between you and Microsoft.
'  
'   To see all Code Samples for Windows Phone, visit http://go.microsoft.com/fwlink/?LinkID=219604 
'  
'
Partial Public Class MainPage
    Inherits PhoneApplicationPage
    ' Constructor
    Public Sub New()
        InitializeComponent()
    End Sub

    Private Sub btn1_Click(ByVal sender As Object, ByVal e As RoutedEventArgs)
        NavigationService.Navigate(New Uri("/Keyboard01.xaml", UriKind.Relative))
    End Sub

    Private Sub btn2_Click(ByVal sender As Object, ByVal e As RoutedEventArgs)
        NavigationService.Navigate(New Uri("/Keyboard02.xaml", UriKind.Relative))
    End Sub

    Private Sub btn3_Click(ByVal sender As Object, ByVal e As RoutedEventArgs)
        NavigationService.Navigate(New Uri("/Keyboard03.xaml", UriKind.Relative))
    End Sub

    Private Sub btn4_Click(ByVal sender As Object, ByVal e As RoutedEventArgs)
        NavigationService.Navigate(New Uri("/Keyboard04.xaml", UriKind.Relative))
    End Sub

    Private Sub btn5_Click(ByVal sender As Object, ByVal e As RoutedEventArgs)
        NavigationService.Navigate(New Uri("/Keyboard05.xaml", UriKind.Relative))
    End Sub

    Private Sub btn6_Click(ByVal sender As Object, ByVal e As RoutedEventArgs)
        NavigationService.Navigate(New Uri("/Keyboard06.xaml", UriKind.Relative))
    End Sub

    Private Sub btn7_Click(ByVal sender As Object, ByVal e As RoutedEventArgs)
        NavigationService.Navigate(New Uri("/Keyboard07.xaml", UriKind.Relative))
    End Sub

    Private Sub btn8_Click(ByVal sender As Object, ByVal e As RoutedEventArgs)
        NavigationService.Navigate(New Uri("/Keyboard08.xaml", UriKind.Relative))
    End Sub

    Private Sub btn9_Click(ByVal sender As Object, ByVal e As RoutedEventArgs)
        NavigationService.Navigate(New Uri("/Keyboard09.xaml", UriKind.Relative))
    End Sub

    Private Sub btn10_Click(ByVal sender As Object, ByVal e As RoutedEventArgs)
        NavigationService.Navigate(New Uri("/Keyboard10.xaml", UriKind.Relative))
    End Sub
End Class
